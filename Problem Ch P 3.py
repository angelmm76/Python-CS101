...
def cellular_automaton(string, pattern, n):
    l_pattern = get_list_pattern(pattern)
    for k in range(n):
        string = cell_aut_one(string, l_pattern)
    return string
    
def cell_aut_one(string, l_pattern):
    res = ''
    trios = ['...', '..x', '.x.', '.xx', 'x..', 'x.x', 'xx.', 'xxx']
    rule = {}
    for k in range(len(l_pattern)):
        if l_pattern[k] == 0:
            rule[trios[k]] = '.'
        else:
            rule[trios[k]] = 'x'
    for i in range(len(string)):
        tri = string[i - 1] + string[i] + string[(i + 1) % len(string)]
        res = res + rule[tri]
    return res
        
def get_list_pattern(pattern):
    res, output = [], []
    for i in range(7, -1, -1):
        res.append(pattern // (2**i))
        pattern = pattern % (2**i)
    for i in range(8):
        output.append(res[7 - i])
    return output
         
    
print get_list_pattern(255)
#print get_list_pattern(25)
print get_list_pattern(64)
#print get_list_pattern(1)
print get_list_pattern(21)
