...
def create_data_structure(string_input):
    ...
    number_users = len(list_input) / 2
    #print  list_input, number_users
    word, name = '', ''
    connections, games = [], []
    for i in range(number_users):
        for c in list_input[2 * i]: #connected users
            if c != ' ':
                word += c
            else:
                name = word
                word = ''
                break
        connections = list_input[2 * i][len(name) + 17:]
        games = list_input[2 * i + 1][len(name) + 15:]      
        network[name] = [connections, games] #add user
        name = ''
        connections, games = [], [] 
    return network
...
def get_secondary_connections(network, user):
	...
	return []
    sec = []
    for c in get_connections(network, user):
        for e in get_connections(network, c):
            if e not in sec:
                sec.append(e)
    return sec
...
def connections_in_common(network, user_A, user_B):
    if user_A not in network or user_B not in network:
        return False
    common = 0
    for e in get_connections(network, user_B):
        if e in get_connections(network, user_A):
            common += 1
    return common
...
def path_to_friend(network, user_A, user_B, pre_path = None):
    if pre_path == None:
        pre_path = []
    path = []
    if user_A not in network or user_B not in network:
    ...
    #if user_A not in pre_path:
    pre_path.append(user_A)
    path = list(pre_path)
    for e in get_connections(network, user_A):
        if e not in path:
            if path_to_friend(network, e, user_B, path) != None:
                return [user_A] + path_to_friend(network, e, user_B, path)
    return None
...
def MYOP():
    ...
    print get_connections(net, "Nick")
    ...
    #print connections_in_common(net, "Mercedes", "John") # 2
    #print connections_in_common(net, "Mercedes", "Robin") # 0
    #print connections_in_common(net, "Mercedes", "Ron") # 0
